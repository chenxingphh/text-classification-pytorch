...
def train(model, dataloader, optimizer, epoch):
    ...
    for idx, (token, label) in enumerate(dataloader):
        optimizer.zero_grad()
        start_time = time.time()

        # 前馈传播
        predicted_label = model(token.to(device))
        # 使用label smooth
...
if __name__ == '__main__':
    ...
    # 加载训练集
    train_data = IMDB_Dataset(mode='train', mask=False, dim=dim)
    train_dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=True, )

    # 加载测试集
    test_data = IMDB_Dataset(mode='test', mask=False)
    ...
    model = FastText(vocab_size, embed_dim, num_class, padding_idx, dropout=0.2, embedding_weight=embedding_weight).to(
        device)
    # 损失函数
